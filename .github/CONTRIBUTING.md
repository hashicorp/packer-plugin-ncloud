...
runtime in order to build packer with the Ncloud plugin.
...
## Setting up Ncloud plugin for dev

With Go installed, you can already `go get` the Ncloud plugin and `make dev` in
...
1. Download the Ncloud plugin source (and its dependencies) by running
...
2. When working on the Ncloud plugin, first `cd $GOPATH/src/github.com/hashicorp/packer-plugin-ncloud`
...
3. Make your changes to the Ncloud plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
...
4. After building the Ncloud plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-ncloud/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the Ncloud plugin](https://github.com/hashicorp/packer-plugin-ncloud#fork-destination-box), push your
...
3. One of Ncloud plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Ncloud plugin
project. After following the steps in "Setting up Go to work on the Ncloud plugin":
...
Please only apply the minimal vendor changes to get your PR to work. The Ncloud plugin
...
If the Ncloud Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
